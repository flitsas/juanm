...
export function ReglasRuleFormDialog({
  ...
  const handleSubmit = () => {
    ...
    onSubmit({
      ...
      secretariatContactId: form.secretariatContactId || null,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="reglas-rule-form-title"
      data-testid="reglas-rule-form-dialog"
    >
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-[var(--card)] p-6 shadow-xl">
        <h2 id="reglas-rule-form-title" className="text-xl font-bold text-[var(--deep)]">
          {rule ? "Editar regla dinámica" : "Nueva regla de Derecho de Petición"}
        </h2>
        ...
        <div className="mt-6 space-y-4">
          ...
          <Field label="Descripción" error={undefined}>
            <InputText
          ...
          <Field label="Contacto secretaría (opcional)" error={errors.secretariatContactId}>
            <Dropdown
              value={form.secretariatContactId}
              options={contactOptions}
              onChange={(e) => update({ secretariatContactId: e.value as string })}
              className="flit-field-input w-full"
            />
          </Field>

          <Field label="Asunto correo" error={errors.emailSubject}>
            <InputText
          ...
          <Field label="Cuerpo HTML" error={errors.emailBodyHtml}>
            <InputTextarea
