...
export function useEmailLogs(comparendoId: string | null) {
  return useQuery({
    queryKey: comparendoId
      ? dgcQueryKeys.emailLogs(comparendoId)
      : ["dgc", "emails", "idle"],
    queryFn: () => fetchEmailLogs(comparendoId!),
...
export function useEmailEvidence(emailId: string | null, enabled: boolean) {
  return useQuery({
    queryKey: emailId
      ? dgcQueryKeys.emailEvidence(emailId)
      : ["dgc", "emails", "evidence", "idle"],
    queryFn: () => fetchEmailEvidence(emailId!),
